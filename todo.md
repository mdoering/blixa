...
- **Backlog (non-blocking):** GBIF preflight count=0 → grey; distribution-map "latest run" on Project-page mount (index `col_match_run_project_idx` already exists); one-active-run-per-project guard; audit trail for bulk alternativeId writes; extract shared code/rank derivation helper; ClbMatchClient brace-in-value encoding edge.
