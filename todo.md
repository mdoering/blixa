...
- **Backlog (non-blocking):** GBIF preflight count=0 → grey; audit trail for bulk alternativeId writes; extract shared code/rank derivation helper (ColMatchService vs ColMatchJobService); ClbMatchClient brace-in-value encoding edge; test the DuplicateKeyException race-backstop path.
