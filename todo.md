...
- **Plan 3 — Import**: not started. Spec + plan to be written against the concrete export core. Import must handle: pro-parte re-merge of derived ids `<synId>-<accId>`, status inverse (`provisionally accepted`→UNASSESSED), enum-casing inverse, known losses (sanctioning_author, occurrence_id), whitespace normalization, both combined + split NameUsage forms.
